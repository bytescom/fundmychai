...
export default function Dashboard() {
  ...
  const viewsLoading = false;
  const earnings = { allTime: 12800, current: 7500, change: 12 };
  const pageViews = { current: 1540, change: 8 };
  const recentSupporters = [
    { _id: "1", name: "Rahul S.", amount: 250, created_at: new Date().toISOString(), tierType: "Cutting Chai" },
    { _id: "2", name: "Priya M.", amount: 100, created_at: new Date().toISOString(), tierType: "Masala Chai" },
    { _id: "3", name: "Vikram", amount: 500, created_at: new Date().toISOString(), tierType: "Special Tandoor" }
  ];
            ...
            <div className="flex gap-2 text-xs sm:text-sm text-gray-500 font-medium">
              {timeframeOptions.map((t) => (
                <button
                  ...
                  className={`cursor-pointer pb-1 transition-colors ${earningsTimeframe === t
            ...
            <div className="flex gap-2 text-xs sm:text-sm text-gray-500 font-medium">
              {timeframeOptions.map((t) => (
                <button
                  ...
                  className={`cursor-pointer pb-1 transition-colors ${viewsTimeframe === t
                      ? 'text-gray-900 border-b-2 border-[#da5407]'
                      : 'hover:text-gray-900'
                    }`}
                >
